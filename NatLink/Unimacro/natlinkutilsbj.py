__version__ = "$Revision: 586 $, $Date: 2018-01-21 20:08:54 +0100 (zo, 21 jan 2018) $, $Author: quintijn $"
...
##                                                       lineLen=lineLen, sort=sort))
    ...
    def setPunctuationList(self, name, puncts=None):
        ...
        """
        allPunctsSpoken = self.spokenforms.spoken2punct.keys()
        if puncts is None:
...
##            print 'ask for exclusive: %s'% self.name
    ...
    def makeDefaultInifile(self, inifile=None, enxVersion=None):
        ...
        if enxVersion:
            ...
            self.ini = inivars.IniVars(inifile, returnStrings=1, repairErrors=1)
            ...
            writeAnything(inifile, None, None, u'\n')
...
##            doc.Range().GoTo(-1,Name="unimacrosearch")
    ...
    def getTopOrChild(self, modInfo=None):
    ...
    def doWaitForMouseToStop(self):
        ...
        if actions.do_MOUSEISMOVING():
            if actions.do_WAITMOUSESTOP():
                ...
                raise Exception("Mouse is not steady, so cannot click")
        return 1
