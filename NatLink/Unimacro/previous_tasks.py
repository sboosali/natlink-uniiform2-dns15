__version__ = "$Rev: 561 $ on $sitegeate: 2014-01-27 13:37:09 +0100 (ma, 27 jan 2014) $ by $Author: quintijn $"
...
class ThisGrammar(ancestor):
    iniIgnoreGrammarLists = ['iconcount', 'taskdoccount', 
...
<fileswitch> exported  = ("switch file to") {switchapp};
    ...
    searchRule = "<searchinothertask> exported = search ({taskdoccount}|{application}) | <before> search ({taskdoccount}|{application});"
    ...
    def initialize(self):
        ...
        self.taskdocCounts = range(1, self.maxTaskDocNumber+1)
        ...
        self.setNumbersList('taskdoccount', self.taskdocCounts)
...
#left|up|right|down|lefttop|righttop|rightbottom|leftbottom;
    ...
    def rule_taskswitch(self, words):
        ...
        task ({taskdoccount}|{taskdoccount}<subtask>|{application}|{application}<subtask>|back|{namedtask})
        """
    ...
    def rule_numbereddocument(self, words):
        ...
        document ({taskdoccount} [{documentaction}] | {documentaction})
        """
    ...
    def gotoTask(self, countOrApp):
        ...
        if isinstance(countOrApp, basestring):
            countBack = self.getNumberFromSpoken(countOrApp, self.taskdocCounts) # returns a string or None
...
##        print 'mx, my:', mx, my
    ...
    def fillInstanceVariables(self):
        ...
        """
        self.maxTaskDocNumber = self.ini.getInt('general', 'max task number') or 20
