"$Revision: 587 $, $Date: 2018-01-23 20:52:46 +0100 (di, 23 jan 2018) $, $Author: quintijn $"
...
from utilsqh import convertToUnicode
DEBUG = 0
...
def getLanguage():
    """get current language, put in unimacro version of natlinkmain
    ...
    if not lang:
        ...
        if currentUser:
            natlinkmain.changeCallback('user', currentUser)
            lang = natlinkmain.language
            if not lang:
                print 'warning: no language found with current user %s, set language to "zyx"'% repr(currentUser)
                return 'zyx'
        else:
            print 'warning: no current user, set language to "xyz"'
            return 'xyz'
    return lang

##    l = natlinkmain.language
##    if l == 'nld':
##        return 'xxx'
##    else:
##        return l
...
def getBaseModel():
    ...
    """
    return natlinkmain.BaseModel

def getBaseTopic():
    """get BaseTopic of user

    """
    return natlinkmain.BaseTopic
...
def getDNSVersion():
    ...
    """
    return natlinkmain.DNSVersion

def getDNSuserDirectory():
    """get DNSUserFolder from natlinkmain

    """
    return natlinkmain.DNSuserDirectory
...
def getUser():
    ...
    """
    return natlink.getCurrentUser()[0]
...
def matchModule(modName, wantedTitle = None, modInfo=None, titleExact=0, caseExact=0):
    if wantedTitle and type(wantedTitle) == six.binary_type:
        wantedTitle = convertToUnicode(wantedTitle)
    if modInfo == None:
...
##    print 'modName: %s, basename module: %s'% (modName, getBaseName(modInfo[0]))
    ...
    winTitle = modInfo[1]
    winTitle = convertToUnicode(winTitle)
...
def matchTitle(wantedTitle, modInfo=None, titleExact=0, caseExact=0):
    ...
    progName = convertToUnicode(progName)
...
def getProgName(modInfo=None):
    if not modInfo:
        modInfo = natlink.getCurrentModule()
    return convertToUnicode(getBaseNameLower(modInfo[0]))
...
def getProgInfo(modInfo=None):
    ...
    if modInfo[0]:
        ...
        if isTopWindow(modInfo[2]):
            ...
            toporchild = u'child'
        return convertToUnicode(prog), convertToUnicode(title), toporchild, convertToUnicode(modInfo[2])
    else:
        return u'', u'', u'empty', convertToUnicode(modInfo[2])
...
def hasBraces(t):
    if not (t and type(t) == types.StringType):
        return
    first = '{' in t or '}' in t
    if first:
        if '{' in t[1:]:
...
def doCaps(t):
    for i in  range(len(t)):
        if t[i] in string.lowercase:
            t = t[:i] + string.upper(t[i]) + t[i+1:]
    return t
...
def waitForWindowTitle(titleName, nWait=50, waitingTime=0.05, comingFrom=None):
...
def waitForNewWindow(nWait=50, waitingTime=0.05, comingFrom=None):
    ...
    for i in range(nWait):
        ...
        modInfo = natlink.getCurrentModule()
        if modInfo[2] and modInfo[2] <> windowHandle:
...
def waitForNewWindowTitle(nWait=50, waitingTime=0.05, comingFrom=None):
    if windowHandle==None:
        raise NatlinkCommandError, "waitForNewWindow, no valid old windowHandle, do a rememberWindow() first"
    for i in range(nWait):
        if waitingCanceled:
...
def returnToWindow(nWait=50, waitingTime=0.05, winHandle=None):
    if not winHandle:
        if not windowHandle:
            raise NatlinkCommandError("returnToWindow without valid window handle, run rememberWindow() first")
        else:
            winHandle = windowHandle
    if not SetForegroundWindow(winHandle):
        print 'setting foreground window failed: %s'% winHandle
        return # this call pwork on vista any more....

    result = win32gui.SetWindowPos( winHandle, win32con.HWND_TOP, 0,0,0,0,  win32con.SWP_NOMOVE | win32con.SWP_NOSIZE )
## not clear what happens here.  Take both calls, hopefully one of them works.
## not clear what happens here.  Take both calls, hopefully one of them works.
##    if result:
##        return
##    print 'returnToWindow to %s failed'% winHandle
####    natqh.SetForegroundWindow(winHandle)
##    for i in range(nWait):
##        if waitingCanceled:
##            clearTrayIcon()
##            print 'waiting canceled'
##            return
##        modInfo = natlink.getCurrentModule()
##        if modInfo[1] and modInfo[2] == winHandle:
##            break
##        setTrayIcon('waiting')
##        Wait(waitingTime)
##    else:
##        clearTrayIcon()
##        raise NatlinkCommandTimeOut("returnToWindow, window not found: %s"% winHandle)
##    # ok, ready:
##    clearTrayIcon()
...
def arg2IntFloatOrString(arg):
    if reint.match(arg): return  string.atoi(arg)
    if refloat.match(arg): return  string.atof(arg)
    return arg
...
def SetForegroundWindow(h):
    ...
    if h == oldH:
        return 1
##        print 'switch to %s, old was: %s'% (h, natqh.windowHandle)zes
    if not win32gui.IsWindow(h):
        print 'SetForegroundWindow: not a window: %s'% h
        return
    if win32gui.IsIconic(h):
        monitorfunctions.restore_window(h)
    try:
    ...
    try:
        ...
        if details[0] in [0, 183]:
            print 'could not bring to foreground (after selecting the dragonbar): %s'% h
            return
        else:
            raise
    for i in range(4):
        ...
        if newH == h:
            return 1
        Wait()
    print 'could not bring wanted window in front, wanted: %s, got: %s'% (h, newH)
