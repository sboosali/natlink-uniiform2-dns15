...
class UnittestActions(TestCaseWithHelpers.TestCaseWithHelpers):
    ...
    def testKeystroke(self):
        ...
        keystroke("{ctrl+x}")
        #keystroke(r"\begin {")
        #keystroke("{\begin {document}")


    def tttestSelectWord(self):
        """test in foreground window the functions for select word
        """
        natqh.rememberWindow()
        if not action("YESNO choose yes and select an empty window if you want to test SELECTWORD"):
            return
        if not natqh.waitForNewWindowTitle(waitingTime=0.1):
            print 'no other window choosen'
            return
        time.sleep(1)
        prog, title, toporchild, windowHandle = natqh.getProgInfo()
        print 'test in %s (title: %s, toporchild: %s)'% (prog, title, toporchild)
        keystroke("this is test selectword{left 10}")
        time.sleep(1)
        result = action("SELECTWORD")
                
        action("MSG %s"% result)

    def tttestAutoHotKey(self):
        """test ahk scripts
        """
