__version__ = "$Rev: 585 $ on $Date: 2018-01-21 18:49:10 +0100 (zo, 21 jan 2018) $ by $Author: quintijn $"
        ...
        if self.hasCommon(words,'window'):
            ...
            if toporchild == 'top':
                if not overruleIsTop:
                    T.append('****** but should be treated as child window according to actions.ini option "top behaves like child"')
            else:
                if overruleIsTop:
                    T.append('****** but should be treated as top window according to actions.ini option "top behaves like child"')
            T.append('')
            ...
            T.append('Natlink/Unimacro userDirectory:\t%s'% natqh.getUnimacroDirectory())
