__version__ = "$Revision: 587 $, $Date: 2018-01-23 20:52:46 +0100 (di, 23 jan 2018) $, $Author: quintijn $"
...
if os.path.isfile(inifile):
    ...
    raise ActionError('no inifile found for actions!')
...
def doAction(action, completeAction=None, pauseBA=None, pauseBK=None,
        ...
        if action.find('USC') >= 0:
            ...
            for a in aList:
                if a.find('USC') >= 0:
                    ...
                    bList.append(a)
            aList = map(string.strip, filter(None, bList))
    ...
    assert type(action) == types.StringType
    ...
    if action.find('(') > 0 and action.strip().endswith(')'):
        com, rest = tuple(map(string.strip, action.split('(', 1)))
        rest = rest.strip()[:-1]
    elif action.find(' ') > 0:
        com, rest = tuple(map(string.strip, action.split(' ',1)))
    else:
    ...
    if metaActions.search(action): # action contains meta actions
        As = map(string.strip, metaActions.split(action))
...
def doKeystroke(action, hardKeys=None, pauseBK=None,
    ...
    if type(hardKeys) == types.StringType:
        hardKeys = [hardKeys]
    elif hardKeys == 1:
        hardKeys = ['all']
    elif hardKeys == 0:
...
def getSectionList(progInfo=None):
    ...
    prog, title, topchild, windowHandle = progInfo
    if debug > 5:
        D('search for prog: %s and title: %s' %
                    (prog, title))
        D('type prog: %s, type title: %s'% (type(prog), type(title)))
    prog = convertToUnicode(prog)
    title = convertToUnicode(title)
    l1 = ini.getSectionsWithPrefix(prog, title)
    l2 = ini.getSectionsWithPrefix(u'default', title)
    l = l1 + l2
    if debug > 5: D('section list with progInfo: %s: %s' %
                    (progInfo, l))
...
def do_SCLIP(*s):
    ...
    total = ', '.join(natqh.convertToUnicode(part) for part in s)
    # print 'total: %s type: %s'% (total, type(total))
    #if len(s) > 1:
...
def do_SELECTWORD(count=1, direction=None):
    ...
    natqh.visibleWait()
    t = natqh.getClipboard()
    if len(t) == 1 and t in string.letters:
        pass
    elif direction == 'left':
...
def do_U(n):
    ...
    if Code <256:
        ...
        natut.playString(chr(Code), 0x100)
        return
    u = unichr(Code)
...
def matchProgTitleWithDict(prog, title, Dict, matchPart=None):
    ...
    if prog in Dict:
        ...
        if isinstance(titles, basestring):
            titles = [titles]
        titles = map(string.lower, titles)
...
def do_WINKEY(letter=None):
    ...
    try:
        if not letter is None:
            if type(letter) == types.StringType:
                do_SSK(letter)
            else:
