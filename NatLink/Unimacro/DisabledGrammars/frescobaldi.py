...
class ThisGrammar(ancestor):
    ...
    def rule_testgetprevnext(self, words):
        ...
        if self.hasSelection:
            ...
            print 'justLeft: %s, justRight: %s'% (repr(self.justLeft), repr(self.justRight))
    ...
    def rule_rythmicpattern(self, words):
        """'rythmic pattern' ({rythms}|{rythms} point)+;
        """
        self.rythmicPattern = []
        rythm = ""
        for w in words:
            if self.hasCommon(w, "point"):
                ...
                if rythm:
                    self.rythmicPattern.append (rythm)
                rythm = self.getFromInifile(w, 'rythms')
        if rythm:
            self.rythmicPattern.append (rythm)
        print 'rythmicPattern: %s'% self.rythmicPattern
        self.rythmicPatternIndex = 0
    ...
    def finishNote(self):
        ...
        if t:
            ...
            if self.rythmicPattern:
                t.append(self.rythmicPattern[self.rythmicPatternIndex])
                self.rythmicPatternIndex += 1
                if self.rythmicPatternIndex == len(self.rythmicPattern):
                    self.rythmicPatternIndex = 0
